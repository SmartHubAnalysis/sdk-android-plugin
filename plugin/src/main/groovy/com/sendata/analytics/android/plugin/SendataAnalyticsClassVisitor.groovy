...
import com.sendata.analytics.android.plugin.hook.SendataPushInjected
...
class SendataAnalyticsClassVisitor extends ClassVisitor {
    ...
    private SendataAnalyticsTransformHelper transformHelper

    private ClassNameAnalytics classNameAnalytics

    private ArrayList<SendataAnalyticsMethodCell> methodCells = new ArrayList<>()

    private int version

    private HashMap<String, SendataAnalyticsMethodCell> mLambdaMethodCells = new HashMap<>()
    ...
    SendataAnalyticsClassVisitor(final ClassVisitor classVisitor, ClassNameAnalytics classNameAnalytics, SendataAnalyticsTransformHelper transformHelper) {
        super(SendataAnalyticsUtil.ASM_VERSION, classVisitor)
    ...
    void visit(int version, int access, String name, String signature, String superName, String[] interfaces) {
        ...
        isExtendsActivity = SendataAnalyticsUtil.isInstanceOfActivity(superName)
        if (Logger.debug) {
    ...
    void visitEnd() {
        ...
        if (SendataAnalyticsUtil.isInstanceOfFragment(mSuperName)) {
            MethodVisitor mv
            // 添加剩下的方法，确保super.onHiddenChanged(hidden);等先被调用
            Iterator<Map.Entry<String, SendataAnalyticsMethodCell>> iterator = SendataAnalyticsHookConfig.FRAGMENT_METHODS.entrySet().iterator()
            while (iterator.hasNext()) {
                Map.Entry<String, SendataAnalyticsMethodCell> entry = iterator.next()
                String key = entry.getKey()
                SendataAnalyticsMethodCell methodCell = entry.getValue()
                ...
                visitMethodWithLoadedParams(mv, Opcodes.INVOKESTATIC, SendataAnalyticsHookConfig.SENDATA_ANALYTICS_API, methodCell.agentName, methodCell.agentDesc, methodCell.paramsStart, methodCell.paramsCount, methodCell.opcodes)
                mv.visitInsn(Opcodes.RETURN)
                mv.visitMaxs(methodCell.paramsCount, methodCell.paramsCount)
                mv.visitEnd()
                mv.visitAnnotation("Lcom/sendata/analytics/android/sdk/SendataInstrumented;", false)
            }
        }

        for (cell in methodCells) {
            transformHelper.sendataAnalyticsHookConfig."${cell.agentName}"(classVisitor, cell)
        }
        if (Logger.debug) {
    ...
    FieldVisitor visitField(int access, String name, String descriptor, String signature, Object value) {
        if (classNameAnalytics.isSendataAPI) {
            if ('VERSION' == name) {
                String version = (String) value
                if (SendataAnalyticsUtil.compareVersion(SendataAnalyticsTransform.MIN_SDK_VERSION, version) > 0) {
                    String errMessage = "你目前集成的Smarthub埋点 SDK 版本号为 v${version}，请升级到 v${SendataAnalyticsTransform.MIN_SDK_VERSION} 及以上的版本。详情请参考：https://github.com/sendata/sa-sdk-android"
                ...
                if (minPluginVersion != "" && minPluginVersion != null) {
                    if (SendataAnalyticsUtil.compareVersion(SendataAnalyticsTransform.VERSION, minPluginVersion) < 0) {
                        String errMessage = "你目前集成的Smarthub插件版本号为 v${SendataAnalyticsTransform.VERSION}，请升级到 v${minPluginVersion} 及以上的版本。详情请参考：https://github.com/sendata/sa-sdk-android-plugin2"
                        Logger.error(errMessage)
                        throw new Error(errMessage)
                    }
                }
    ...
    MethodVisitor visitMethod(int access, String name, String desc, String signature, String[] exceptions) {
        ...
        if (transformHelper.extension != null && transformHelper.extension.autoHandleWebView && transformHelper.urlClassLoader != null) {
            methodVisitor = new SendataAnalyticsWebViewMethodVisitor(methodVisitor, access, name, desc, transformHelper, mClassName, mSuperName)
        }
        SendataAnalyticsDefaultMethodVisitor sendataAnalyticsDefaultMethodVisitor = new SendataAnalyticsDefaultMethodVisitor(methodVisitor, access, name, desc) {
            boolean isSendataTrackViewOnClickAnnotation = false
            boolean isSendataIgnoreTrackOnClick = false
            ...
            void visitEnd() {
                ...
                if (isHasTracked) {
                    ...
                    visitAnnotation("Lcom/sendata/analytics/android/sdk/SendataInstrumented;", false)
            ...
            void visitInvokeDynamicInsn(String name1, String desc1, Handle bsm, Object... bsmArgs) {
                ...
                try {
                    ...
                    SendataAnalyticsMethodCell sendataAnalyticsMethodCell = SendataAnalyticsHookConfig.LAMBDA_METHODS.get(Type.getReturnType(desc1).getDescriptor() + name1 + desc2)
                    if (sendataAnalyticsMethodCell != null) {
                        Handle it = (Handle) bsmArgs[1]
                        mLambdaMethodCells.put(it.name + it.desc, sendataAnalyticsMethodCell)
                    }
                } catch (Exception e) {
                    e.printStackTrace()
                }
            ...
            protected void onMethodEnter() {
                ...
                nameDesc = name + desc
                // Hook Push
                if (!SendataAnalyticsUtil.isStatic(access) && !transformHelper.extension.disableTrackPush) {
                    SendataPushInjected.handleJPush(methodVisitor, mSuperName, mClassName, nameDesc)
                    SendataPushInjected.handleMeizuPush(methodVisitor, mSuperName, mClassName, nameDesc)
                }

                pubAndNoStaticAccess = SendataAnalyticsUtil.isPublic(access) && !SendataAnalyticsUtil.isStatic(access)
                protectedAndNotStaticAccess = SendataAnalyticsUtil.isProtected(access) && !SendataAnalyticsUtil.isStatic(access)
                if (pubAndNoStaticAccess) {
                    if ((nameDesc == 'onClick(Landroid/view/View;)V')) {
                        ...
                        localIds.add(third)
                    } else if (SendataAnalyticsUtil.isInstanceOfFragment(mSuperName)
                            && SendataAnalyticsHookConfig.FRAGMENT_METHODS.get(nameDesc) != null) {
                        SendataAnalyticsMethodCell sendataAnalyticsMethodCell = SendataAnalyticsHookConfig.FRAGMENT_METHODS.get(nameDesc)
                        localIds = new ArrayList<>()
                        Type[] types = Type.getArgumentTypes(desc)
                        for (int i = 1; i < sendataAnalyticsMethodCell.paramsCount; i++) {
                            int localId = newLocal(types[i - 1])
                            methodVisitor.visitVarInsn(sendataAnalyticsMethodCell.opcodes.get(i), i)
                            methodVisitor.visitVarInsn(SendataAnalyticsUtil.convertOpcodes(sendataAnalyticsMethodCell.opcodes.get(i)), localId)
                        ...
                        methodVisitor.visitVarInsn(ISTORE, secondLocalId)
                        localIds.add(secondLocalId)
                    } else if (SendataAnalyticsUtil.isTargetMenuMethodDesc(nameDesc)) {
                        localIds = new ArrayList<>()
                ...
                if (transformHelper.extension.lambdaEnabled) {
                    SendataAnalyticsMethodCell lambdaMethodCell = mLambdaMethodCells.get(nameDesc)
                    if (lambdaMethodCell != null) {
                        //判断是否是在采样中，在采样中才会处理或者开关打开也统一处理
                        if (transformHelper.extension.lambdaParamOptimize || SendataAnalyticsHookConfig.SAMPLING_LAMBDA_METHODS.contains(lambdaMethodCell)) {
                            ...
                            boolean isStaticMethod = SendataAnalyticsUtil.isStatic(access)
                            localIds = new ArrayList<>()
                            for (int i = paramStart; i < paramStart + lambdaMethodCell.paramsCount; i++) {
                                ...
                                methodVisitor.visitVarInsn(SendataAnalyticsUtil.convertOpcodes(lambdaMethodCell.opcodes.get(i - paramStart)), localId)
                                localIds.add(localId)
                            }
                        }
                    }
            ...
            void visitFieldInsn(int opcode, String owner, String fieldName, String fieldDesc) {
                if (classNameAnalytics.isSendataAPI && "ANDROID_PLUGIN_VERSION" == fieldName && opcode == PUTSTATIC) {
                    methodVisitor.visitLdcInsn(SendataAnalyticsTransform.VERSION)
                }
                super.visitFieldInsn(opcode, owner, fieldName, fieldDesc)
            }

            void handleCode() {
                if (isHasInstrumented || classNameAnalytics.isSendataAPI) {
                    return
                }
                ...
                if (SendataAnalyticsUtil.isInstanceOfFragment(mSuperName)) {
                    ...
                    if (sendataAnalyticsMethodCell != null) {
                        ...
                        methodVisitor.visitVarInsn(ALOAD, 0)
                        for (int i = 1; i < sendataAnalyticsMethodCell.paramsCount; i++) {
                            methodVisitor.visitVarInsn(sendataAnalyticsMethodCell.opcodes.get(i), localIds[i - 1])
                        }
                        methodVisitor.visitMethodInsn(INVOKESTATIC, SendataAnalyticsHookConfig.SENDATA_ANALYTICS_API, sendataAnalyticsMethodCell.agentName, sendataAnalyticsMethodCell.agentDesc, false)
                        isHasTracked = true
                        return
                    }
                }

                if (isSendataIgnoreTrackOnClick) {
                    return
                }
                    ...
                    if (lambdaMethodCell != null) {
                        ...
                        if (!isStaticMethod) {
                            if (lambdaMethodCell.desc == '(Landroid/view/MenuItem;)Z') {
                                ...
                                methodVisitor.visitMethodInsn(Opcodes.INVOKESTATIC, SendataAnalyticsHookConfig.SENDATA_ANALYTICS_API, lambdaMethodCell.agentName, '(Ljava/lang/Object;Landroid/view/MenuItem;)V', false)
                                isHasTracked = true
                                return
                            }
                        }
                        ...
                        methodVisitor.visitMethodInsn(Opcodes.INVOKESTATIC, SendataAnalyticsHookConfig.SENDATA_ANALYTICS_API, lambdaMethodCell.agentName, lambdaMethodCell.agentDesc, false)
                        isHasTracked = true
                        return
                    }
                }
                ...
                if (!pubAndNoStaticAccess) {
                    ...
                    if (protectedAndNotStaticAccess) {
                        if (nameDesc == "onListItemClick(Landroid/widget/ListView;Landroid/view/View;IJ)V") {
                            ...
                            methodVisitor.visitMethodInsn(INVOKESTATIC, SendataAnalyticsHookConfig.SENDATA_ANALYTICS_API, "trackListView", "(Landroid/widget/AdapterView;Landroid/view/View;I)V", false)
                            isHasTracked = true
                            return
                        }
                    }
                ...
                if (isAndroidTv && isExtendsActivity && nameDesc == 'dispatchKeyEvent(Landroid/view/KeyEvent;)Z') {
                    methodVisitor.visitVarInsn(ALOAD, 0)
                    methodVisitor.visitVarInsn(ALOAD, 1)
                    methodVisitor.visitMethodInsn(INVOKESTATIC, SendataAnalyticsHookConfig.SENDATA_ANALYTICS_API, "trackViewOnClick", "(Landroid/app/Activity;Landroid/view/KeyEvent;)V", false)
                    isHasTracked = true
                    return
                }
                 ...
                 */
                if (SendataAnalyticsUtil.isTargetMenuMethodDesc(nameDesc)) {
                    methodVisitor.visitVarInsn(ALOAD, localIds[0])
                    methodVisitor.visitVarInsn(ALOAD, localIds[1])
                    methodVisitor.visitMethodInsn(INVOKESTATIC, SendataAnalyticsHookConfig.SENDATA_ANALYTICS_API, "trackMenuItem", "(Ljava/lang/Object;Landroid/view/MenuItem;)V", false)
                    isHasTracked = true
                    return
                }
                ...
                if (nameDesc == 'onDrawerOpened(Landroid/view/View;)V') {
                    methodVisitor.visitVarInsn(ALOAD, 1)
                    methodVisitor.visitMethodInsn(INVOKESTATIC, SendataAnalyticsHookConfig.SENDATA_ANALYTICS_API, "trackDrawerOpened", "(Landroid/view/View;)V", false)
                    isHasTracked = true
                    return
                } else if (nameDesc == 'onDrawerClosed(Landroid/view/View;)V') {
                    methodVisitor.visitVarInsn(ALOAD, 1)
                    methodVisitor.visitMethodInsn(INVOKESTATIC, SendataAnalyticsHookConfig.SENDATA_ANALYTICS_API, "trackDrawerClosed", "(Landroid/view/View;)V", false)
                    isHasTracked = true
                    return
                }
                ...
                if (!SendataAnalyticsUtil.isTargetClassInSpecial(mClassName)) {
                ...
                if (isSendataTrackViewOnClickAnnotation && desc == '(Landroid/view/View;)V') {
                    trackViewOnClick(methodVisitor, 1)
                    isHasTracked = true
                    return
                }
                ...
                if (eventName != null && eventName.length() != 0) {
                    ...
                    methodVisitor.visitMethodInsn(INVOKESTATIC, SendataAnalyticsHookConfig.SENDATA_ANALYTICS_API, "track", "(Ljava/lang/String;Ljava/lang/String;)V", false)
                    isHasTracked = true
                    return
                }
                ...
                if (mInterfaces != null && mInterfaces.length > 0) {
                    if (isOnItemClickMethod && mInterfaces.contains('android/widget/AdapterView$OnItemClickListener')) {
                        ...
                        SendataAnalyticsMethodCell sendataAnalyticsMethodCell = SendataAnalyticsHookConfig.INTERFACE_METHODS
                        ...
                        for (interfaceName in mInterfaces) {
                            SendataAnalyticsMethodCell sendataAnalyticsMethodCell = SendataAnalyticsHookConfig.INTERFACE_METHODS.get(interfaceName + nameDesc)
                            if (sendataAnalyticsMethodCell != null) {
                                methodVisitor.visitVarInsn(ALOAD, localIds.get(0))
                                methodVisitor.visitMethodInsn(INVOKESTATIC, SendataAnalyticsHookConfig.SENDATA_ANALYTICS_API, sendataAnalyticsMethodCell.agentName, sendataAnalyticsMethodCell.agentDesc, false)
                                isHasTracked = true
                        ...
                        for (interfaceName in mInterfaces) {
                            SendataAnalyticsMethodCell sendataAnalyticsMethodCell = SendataAnalyticsHookConfig.INTERFACE_METHODS.get(interfaceName + nameDesc)
                            if (sendataAnalyticsMethodCell != null) {
                                visitMethodWithLoadedParams(methodVisitor, INVOKESTATIC, SendataAnalyticsHookConfig.SENDATA_ANALYTICS_API, sendataAnalyticsMethodCell.agentName, sendataAnalyticsMethodCell.agentDesc, sendataAnalyticsMethodCell.paramsStart, sendataAnalyticsMethodCell.paramsCount, sendataAnalyticsMethodCell.opcodes)
                                isHasTracked = true
                                return
                            }
                        }
            ...
            void handleClassMethod(String className, String nameDesc) {
                SendataAnalyticsMethodCell sendataAnalyticsMethodCell = SendataAnalyticsHookConfig.CLASS_METHODS.get(className + nameDesc)
                if (sendataAnalyticsMethodCell != null) {
                    visitMethodWithLoadedParams(methodVisitor, INVOKESTATIC, SendataAnalyticsHookConfig.SENDATA_ANALYTICS_API, sendataAnalyticsMethodCell.agentName, sendataAnalyticsMethodCell.agentDesc, sendataAnalyticsMethodCell.paramsStart, sendataAnalyticsMethodCell.paramsCount, sendataAnalyticsMethodCell.opcodes)
                    isHasTracked = true
                }
            ...
            boolean handleRN() {
                ...
                switch (transformHelper.rnState) {
                    case SendataAnalyticsTransformHelper.RN_STATE.NOT_FOUND:
                        break
                    case SendataAnalyticsTransformHelper.RN_STATE.HAS_VERSION:
                        if (transformHelper.rnVersion > '2.0.0' && mSuperName == "com/facebook/react/uimanager/ViewGroupManager"
                            ...
                            methodVisitor.visitVarInsn(ALOAD, 2)
                            methodVisitor.visitVarInsn(ILOAD, 3)
                            methodVisitor.visitMethodInsn(INVOKESTATIC, "com/sendata/analytics/RNAgent", "addView", "(Landroid/view/View;I)V", false)
                            result = true
                        }
                        ...
                        if (nameDesc == 'handleTouchEvent(Landroid/view/MotionEvent;Lcom/facebook/react/uimanager/events/EventDispatcher;)V' && mClassName == 'com/facebook/react/uimanager/JSTouchDispatcher') {
                            ...
                            methodVisitor.visitVarInsn(ALOAD, 1)
                            methodVisitor.visitVarInsn(ALOAD, 2)
                            methodVisitor.visitMethodInsn(INVOKESTATIC, "com/sendata/analytics/RNAgent", "handleTouchEvent", "(Lcom/facebook/react/uimanager/JSTouchDispatcher;Landroid/view/MotionEvent;Lcom/facebook/react/uimanager/events/EventDispatcher;)V", false)
                            result = true
                        }
                        break
                    case SendataAnalyticsTransformHelper.RN_STATE.NO_VERSION:
                        if (nameDesc == 'setJSResponder(IIZ)V' && mClassName == 'com/facebook/react/uimanager/NativeViewHierarchyManager') {
                            methodVisitor.visitVarInsn(ALOAD, 0)
                            methodVisitor.visitVarInsn(ILOAD, 1)
                            methodVisitor.visitVarInsn(ILOAD, 2)
                            methodVisitor.visitVarInsn(ILOAD, 3)
                            methodVisitor.visitMethodInsn(INVOKESTATIC, SendataAnalyticsHookConfig.SENDATA_ANALYTICS_API, "trackRN", "(Ljava/lang/Object;IIZ)V", false)
                            result = true
                        }
                        break
                }
            ...
            void trackViewOnClick(MethodVisitor mv, int index) {
                ...
                mv.visitMethodInsn(INVOKESTATIC, SendataAnalyticsHookConfig.SENDATA_ANALYTICS_API, "trackViewOnClick", "(Landroid/view/View;)V", false)
            }
            ...
            AnnotationVisitor visitAnnotation(String s, boolean b) {
                if (s == 'Lcom/sendata/analytics/android/sdk/SendataTrackViewOnClick;') {
                    isSendataTrackViewOnClickAnnotation = true
                    Logger.info("发现 ${name}${desc} 有注解 @SendataTrackViewOnClick")
                } else if (s == 'Lcom/sendata/analytics/android/sdk/SendataIgnoreTrackOnClick;') {
                    isSendataIgnoreTrackOnClick = true
                    Logger.info("发现 ${name}${desc} 有注解 @SendataIgnoreTrackOnClick")
                } else if (s == 'Lcom/sendata/analytics/android/sdk/SendataInstrumented;') {
                    isHasInstrumented = true
                } else if (s == 'Lcom/sendata/analytics/android/sdk/SendataTrackEvent;') {
                    return new AnnotationVisitor(SendataAnalyticsUtil.ASM_VERSION) {
                        @Override
        ...
        if (version <= Opcodes.V1_5) {
            return new SendataAnalyticsJSRAdapter(SendataAnalyticsUtil.ASM_VERSION, sendataAnalyticsDefaultMethodVisitor, access, name, desc, signature, exceptions)
        }
        return sendataAnalyticsDefaultMethodVisitor
    }
