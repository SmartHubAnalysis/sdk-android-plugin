...
class SendataAnalyticsTransform extends Transform {
    private SendataAnalyticsTransformHelper transformHelper
    public static final String VERSION = "1.0.0"
    public static final String MIN_SDK_VERSION = "1.0.0"
    private WaitableExecutor waitableExecutor
    private URLClassLoader urlClassLoader
    // “com.sendata.analytics.android.sdk.SendataAPI” 类所在路径
    private String sendataSdkJarPath
    private volatile boolean isFoundSDKJar = false

    SendataAnalyticsTransform(SendataAnalyticsTransformHelper transformHelper) {
        this.transformHelper = transformHelper
        if (!transformHelper.disableSendataAnalyticsMultiThread) {
    ...
    @Override
    String getName() {
        return "sendataAnalyticsAutoTrack"
    }
    ...
    boolean isIncremental() {
        return !transformHelper.disableSendataAnalyticsIncremental
    }
    ...
    private void transformClass(Context context, Collection<TransformInput> inputs, TransformOutputProvider outputProvider, boolean isIncremental)
        ...
        println("[SendataAnalytics]: 此次编译共耗时:${System.currentTimeMillis() - startTime}毫秒")
    }
    ...
    private void beforeTransform(TransformInvocation transformInvocation) {
        ...
        println("[SendataAnalytics]: 是否开启多线程编译:${!transformHelper.disableSendataAnalyticsMultiThread}")
        println("[SendataAnalytics]: 是否开启增量编译:${!transformHelper.disableSendataAnalyticsIncremental}")
        println("[SendataAnalytics]: 此次是否增量编译:$transformInvocation.incremental")
        println("[SendataAnalytics]: 是否在方法进入时插入代码:${transformHelper.isHookOnMethodEnter}")
    ...
    private void checkSendataSDK() {
        try {
            Class sdkClazz = urlClassLoader.loadClass("com.sendata.analytics.android.sdk.SendataAPI")
            ProtectionDomain pd = sdkClazz.getProtectionDomain()
            CodeSource cs = pd.getCodeSource()
            sendataSdkJarPath = cs.getLocation().toURI().getPath()
        } catch (Throwable throwable) {
            Logger.error("Can not load 'com.sendata.analytics.android.sdk.SendataAPI' class: ${throwable.localizedMessage}")
        }
    }

    private void checkRNState() {
        try {
            Class rnClazz = urlClassLoader.loadClass("com.sendata.analytics.RNSendataAnalyticsPackage")
            try {
                Field versionField = rnClazz.getDeclaredField("VERSION")
                versionField.setAccessible(true)
                transformHelper.rnVersion = versionField.get(null) as String
                transformHelper.rnState = SendataAnalyticsTransformHelper.RN_STATE.HAS_VERSION
            } catch (Exception e) {
                transformHelper.rnState = SendataAnalyticsTransformHelper.RN_STATE.NO_VERSION
            }
        } catch (Exception e) {
            transformHelper.rnState = SendataAnalyticsTransformHelper.RN_STATE.NOT_FOUND
        }
    }
    ...
    private boolean checkJarValidate(JarInput jarInput) {
        try {
            if (isFoundSDKJar || sendataSdkJarPath == null) {
                return false
            }
            def jarLocation = jarInput.file.toURI().getPath()
            if (sendataSdkJarPath.length() == jarLocation.length() && sendataSdkJarPath == jarLocation) {
                isFoundSDKJar = true
                return true
            } else {
                return false
        ...
        while (enumeration.hasMoreElements()) {
            ...
            if (entryName.endsWith(".DSA") || entryName.endsWith(".SF")) {
                ...
                try {
                    ...
                    sourceClassBytes = SendataAnalyticsUtil.toByteArrayAndAutoCloseStream(inputStream)
                } catch (Exception e) {
    ...
    private byte[] modifyClass(byte[] srcClass, ClassNameAnalytics classNameAnalytics) {
        try {
            ...
            ClassVisitor classVisitor = new SendataAnalyticsClassVisitor(classWriter, classNameAnalytics, transformHelper)
    ...
    private File modifyClassFile(File dir, File classFile, File tempDir) {
        ...
        try {
            ...
            ClassNameAnalytics classNameAnalytics = transformHelper.analytics(className)
            if (classNameAnalytics.isShouldModify) {
                byte[] sourceClassBytes = SendataAnalyticsUtil.toByteArrayAndAutoCloseStream(new FileInputStream(classFile))
