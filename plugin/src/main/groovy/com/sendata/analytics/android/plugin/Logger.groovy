...
class Logger {
    ...
    static void printCopyright() {
        ...
        println("${LogUI.C_BLACK_GREEN.value}" + "###                                                              ###" + "${LogUI.E_NORMAL.value}")
        println("${LogUI.C_BLACK_GREEN.value}" + "###                                                              ###" + "${LogUI.E_NORMAL.value}")
        println("${LogUI.C_BLACK_GREEN.value}" + "###          欢迎使用 SendataAnalytics® (v" + SendataAnalyticsTransform.VERSION + ")编译插件         ###" + "${LogUI.E_NORMAL.value}")
        ...
        println("${LogUI.C_BLACK_GREEN.value}" + "###     https://github.com/sendata/sa-sdk-android-plugin2    ###" + "${LogUI.E_NORMAL.value}")
        println("${LogUI.C_BLACK_GREEN.value}" + "###                                                              ###" + "${LogUI.E_NORMAL.value}")
        println("${LogUI.C_BLACK_GREEN.value}" + "###                                                              ###" + "${LogUI.E_NORMAL.value}")
        println("${LogUI.C_BLACK_GREEN.value}" + "####################################################################" + "${LogUI.E_NORMAL.value}")
        println()
    ...
    def static error(Object msg) {
        try {
            println("${LogUI.C_ERROR.value}[SendataAnalytics]: ${msg}${LogUI.E_NORMAL.value}")
        } catch (Exception e) {
            e.printStackTrace()
        }
    }
    ...
    def static warn(Object msg) {
        try {
            println("${LogUI.C_WARN.value}[SendataAnalytics]: ${msg}${LogUI.E_NORMAL.value}")
        } catch (Exception e) {
            e.printStackTrace()
        }
    }
