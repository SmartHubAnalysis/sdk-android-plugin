 ...
 */
class SendataAnalyticsWebViewMethodVisitor extends AdviceAdapter implements Opcodes {
    ...
    SendataAnalyticsWebViewMethodVisitor(MethodVisitor mv, int access, String name, String desc, SendataAnalyticsTransformHelper transformHelper, String className, String superName) {
        super(SendataAnalyticsUtil.ASM_VERSION, mv, access, name, desc)
            ...
            if (opcode != INVOKESTATIC && TARGET_NAME_DESC.contains(name + desc)) {
                ...
                if (isAssignableWebView(owner)) {
                    ...
                    if (isDCloud(owner, className)) {
                        ...
                        mv.visitMethodInsn(INVOKESTATIC, SendataAnalyticsHookConfig.SENDATA_ANALYTICS_API, name + "2", desc, false)
                    }
                    return
                }
            }
