...
import com.sendata.analytics.android.sdk.util.SADisplayUtil;
...
class DebugModeSelectDialog extends Dialog implements View.OnClickListener {
    ...
    private SendataAPI.DebugMode currentDebugMode;

    DebugModeSelectDialog(Context context, SendataAPI.DebugMode debugMode) {
        super(context);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.sendata_analytics_debug_mode_dialog_content);
        initView();
    ...
    private void initView() {
        //标题:SDK 调试模式选择
        TextView debugModeTitle = findViewById(R.id.sendata_analytics_debug_mode_title);
        debugModeTitle.setText("SDK 调试模式选择");
        //取消
        TextView debugModeCancel = findViewById(R.id.sendata_analytics_debug_mode_cancel);
        debugModeCancel.setText("取消");
        debugModeCancel.setOnClickListener(this);
        debugModeCancel.setFocusable(true);
        //开启调试模式(不导入数据)
        TextView debugModeOnly = findViewById(R.id.sendata_analytics_debug_mode_only);
        debugModeOnly.setText("开启调试模式（不导入数据）");
        debugModeOnly.setOnClickListener(this);
        debugModeOnly.setFocusable(true);
        //"开启调试模式(导入数据)"
        TextView debugModeTrack = findViewById(R.id.sendata_analytics_debug_mode_track);
        debugModeTrack.setText("开启调试模式（导入数据）");
        debugModeTrack.setOnClickListener(this);
        debugModeTrack.setFocusable(true);

        String msg = "调试模式已关闭";
        if (currentDebugMode == SendataAPI.DebugMode.DEBUG_ONLY) {
            msg = "当前为 调试模式（不导入数据）";
        } else if (currentDebugMode == SendataAPI.DebugMode.DEBUG_AND_TRACK) {
            msg = "当前为 测试模式（导入数据）";
        }
        TextView debugModeMessage = findViewById(R.id.sendata_analytics_debug_mode_message);
    ...
    public void onClick(View v) {
        ...
        int id = v.getId();
        if (id == R.id.sendata_analytics_debug_mode_track) {
            onDebugModeDialogClickListener.setDebugMode(this, SendataAPI.DebugMode.DEBUG_AND_TRACK);
        } else if (id == R.id.sendata_analytics_debug_mode_only) {
            onDebugModeDialogClickListener.setDebugMode(this, SendataAPI.DebugMode.DEBUG_ONLY);
        } else if (id == R.id.sendata_analytics_debug_mode_cancel) {
    ...
    interface OnDebugModeViewClickListener {
        ...
        void setDebugMode(Dialog dialog, SendataAPI.DebugMode debugMode);
    }
}
