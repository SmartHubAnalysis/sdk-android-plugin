...
import com.sendata.analytics.android.sdk.data.persistent.PersistentDistinctId;
...
import com.sendata.analytics.android.sdk.data.persistent.PersistentFirstTrackInstallation;
import com.sendata.analytics.android.sdk.data.persistent.PersistentFirstTrackInstallationWithCallback;
import com.sendata.analytics.android.sdk.data.persistent.PersistentLoader;
import com.sendata.analytics.android.sdk.data.persistent.PersistentSuperProperties;
import com.sendata.analytics.android.sdk.deeplink.SendataDeepLinkCallback;
import com.sendata.analytics.android.sdk.encrypt.SendataEncrypt;
import com.sendata.analytics.android.sdk.exceptions.InvalidDataException;
import com.sendata.analytics.android.sdk.internal.api.FragmentAPI;
import com.sendata.analytics.android.sdk.internal.api.IFragmentAPI;
import com.sendata.analytics.android.sdk.internal.rpc.SendataContentObserver;
import com.sendata.analytics.android.sdk.listener.SAEventListener;
import com.sendata.analytics.android.sdk.listener.SAFunctionListener;
import com.sendata.analytics.android.sdk.listener.SAJSListener;
import com.sendata.analytics.android.sdk.remote.BaseSendataSDKRemoteManager;
import com.sendata.analytics.android.sdk.remote.SendataRemoteManager;
import com.sendata.analytics.android.sdk.util.AppInfoUtils;
import com.sendata.analytics.android.sdk.util.ChannelUtils;
import com.sendata.analytics.android.sdk.util.JSONUtils;
import com.sendata.analytics.android.sdk.util.NetworkUtils;
import com.sendata.analytics.android.sdk.util.OaidHelper;
import com.sendata.analytics.android.sdk.util.SADataHelper;
import com.sendata.analytics.android.sdk.util.SAContextManager;
...
import static com.sendata.analytics.android.sdk.util.SADataHelper.assertKey;
import static com.sendata.analytics.android.sdk.util.SADataHelper.assertPropertyTypes;
import static com.sendata.analytics.android.sdk.util.SADataHelper.assertValue;

abstract class AbstractSendataAPI implements ISendataAPI {
    protected static final String TAG = "SA.SendataAPI";
    // SDK版本
    static final String VERSION = BuildConfig.SDK_VERSION;
    // Maps each token to a singleton SendataAPI instance
    protected static final Map<Context, SendataAPI> sInstanceMap = new HashMap<>();
    static boolean mIsMainProcess = false;
    static boolean SHOW_DEBUG_INFO_VIEW = true;
    protected static SendataGPSLocation mGPSLocation;
    /* 远程配置 */
    ...
    /* Debug 模式选项 */
    protected SendataAPI.DebugMode mDebugMode = SendataAPI.DebugMode.DEBUG_OFF;
    /* SDK 自动采集事件 */
    ...
    protected SendataScreenOrientationDetector mOrientationDetector;
    protected SendataDynamicSuperProperties mDynamicSuperPropertiesCallBack;
    protected SimpleDateFormat mIsFirstDayDateFormat;
    protected SendataTrackEventCallBack mTrackEventCallBack;
    ...
    SendataEncrypt mSendataEncrypt;
    protected SendataDeepLinkCallback mDeepLinkCallback;
    // $AppDeeplinkLaunch 是否携带设备信息
    boolean mEnableDeepLinkInstallSource = false;
    BaseSendataSDKRemoteManager mRemoteManager;
    /**
     * 标记是否已经采集了带有插件版本号的事件
     */
    private boolean isTrackEventWithPluginVersion = false;

    public AbstractSendataAPI(Context context, SAConfigOptions configOptions, SendataAPI.DebugMode debugMode) {
        mContext = context;
        ...
        try {
            ...
            mMessages = AnalyticsMessages.getInstance(mContext, (SendataAPI) this);
            mRemoteManager = new SendataRemoteManager((SendataAPI) this);
            //先从缓存中读取 SDKConfig
            ...
            //打开 debug 模式，弹出提示
            if (mDebugMode != SendataAPI.DebugMode.DEBUG_OFF && mIsMainProcess) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
                ...
                final SendataActivityLifecycleCallbacks lifecycleCallbacks =
                        new SendataActivityLifecycleCallbacks((SendataAPI) this, mFirstStart, mFirstDay, context);
            ...
            if (SALog.isLogEnabled()) {
                SALog.i(TAG, String.format(Locale.CHINA, "Initialized the instance of Sendata Analytics SDK with server"
    ...
    public static boolean isSDKDisabled() {
        boolean isSDKDisabled = SendataRemoteManager.isSDKDisabledByRemote();
        if (isSDKDisabled) {
    ...
    SendataDeepLinkCallback getDeepLinkCallback() {
    ...
    public SendataAPI.DebugMode getDebugMode() {
        return mDebugMode;
    }

    public void setDebugMode(SendataAPI.DebugMode debugMode) {
        mDebugMode = debugMode;
        if (debugMode == SendataAPI.DebugMode.DEBUG_OFF) {
            enableLog(false);
    ...
    public BaseSendataSDKRemoteManager getRemoteManager() {
        return mRemoteManager;
    }

    public void setRemoteManager(BaseSendataSDKRemoteManager remoteManager) {
        this.mRemoteManager = remoteManager;
    }

    public SendataEncrypt getSendataEncrypt() {
        return mSendataEncrypt;
    }
        ...
        transformTaskQueue(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    SendataUtils.mergeJSONObject(_properties, profileProperties);
            ...
            try {
                ...
                if (eventType.isTrack()) {
                    ...
                    if (!"$AppEnd".equals(eventName) && !"$AppDeeplinkLaunch".equals(eventName)) {
                        //合并 $latest_utm 属性
                        SendataUtils.mergeJSONObject(ChannelUtils.getLatestUtmProperties(), sendProperties);
                    }
    ...
    protected void trackEventH5(String eventInfo) {
        try {
            ...
            if (eventType.isTrack()) {
                ...
                SendataUtils.mergeJSONObject(ChannelUtils.getLatestUtmProperties(), propertiesObject);
            }
    ...
    protected void initSAConfig(String serverURL, String packageName) {
        ...
        if (mSAConfigOptions.mEnableEncrypt) {
            mSendataEncrypt = new SendataEncrypt(mContext, mSAConfigOptions.mPersistentSecretKey, mSAConfigOptions.mEncryptListeners);
        }

        DbAdapter.getInstance(mContext, packageName, mSendataEncrypt);
        ...
        if (mSAConfigOptions.mInvokeLog) {
            ...
            enableLog(configBundle.getBoolean("com.sendata.analytics.android.EnableLogging",
                    this.mDebugMode != SendataAPI.DebugMode.DEBUG_OFF));
        }

        setServerUrl(serverURL);
        if (mSAConfigOptions.mEnableTrackAppCrash) {
            SendataExceptionHandler.enableAppCrash();
        }

        if (mSAConfigOptions.mFlushInterval == 0) {
            mSAConfigOptions.setFlushInterval(configBundle.getInt("com.sendata.analytics.android.FlushInterval",
                    15000));
        }

        if (mSAConfigOptions.mFlushBulkSize == 0) {
            mSAConfigOptions.setFlushBulkSize(configBundle.getInt("com.sendata.analytics.android.FlushBulkSize",
                    100));
        }
        ...
        this.mAutoTrack = configBundle.getBoolean("com.sendata.analytics.android.AutoTrack",
                false);
        ...
        if (!mSAConfigOptions.mInvokeHeatMapEnabled) {
            mSAConfigOptions.mHeatMapEnabled = configBundle.getBoolean("com.sendata.analytics.android.HeatMap",
                    false);
        }

        if (!mSAConfigOptions.mInvokeHeatMapConfirmDialog) {
            mSAConfigOptions.mHeatMapConfirmDialogEnabled = configBundle.getBoolean("com.sendata.analytics.android.EnableHeatMapConfirmDialog",
                    false);
        }

        if (!mSAConfigOptions.mInvokeVisualizedEnabled) {
            mSAConfigOptions.mVisualizedEnabled = configBundle.getBoolean("com.sendata.analytics.android.VisualizedAutoTrack",
                    false);
        }

        if (!mSAConfigOptions.mInvokeVisualizedConfirmDialog) {
            mSAConfigOptions.mVisualizedConfirmDialogEnabled = configBundle.getBoolean("com.sendata.analytics.android.EnableVisualizedAutoTrackConfirmDialog",
                    false);
        }
        ...
        SHOW_DEBUG_INFO_VIEW = configBundle.getBoolean("com.sendata.analytics.android.ShowDebugInfoView",
                true);

        this.mDisableDefaultRemoteConfig = configBundle.getBoolean("com.sendata.analytics.android.DisableDefaultRemoteConfig",
                false);
        ...
        this.mDisableTrackDeviceId = configBundle.getBoolean("com.sendata.analytics.android.DisableTrackDeviceId",
                false);
    }
    ...
    private void mergerDynamicAndSuperProperties(JSONObject eventProperty, JSONObject dynamicProperty) {
        ...
        JSONObject removeDuplicateSuperProperties = SendataUtils.mergeSuperJSONObject(dynamicProperty, superProperties);
        SendataUtils.mergeJSONObject(removeDuplicateSuperProperties, eventProperty);
    }

    private void showDebugModeWarning() {
        try {
            if (mDebugMode == SendataAPI.DebugMode.DEBUG_OFF) {
                return;
            }
            ...
            handler.post(new Runnable() {
                ...
                public void run() {
                    String info = null;
                    if (mDebugMode == SendataAPI.DebugMode.DEBUG_ONLY) {
                        info = "现在您打开了 Sendata SDK 的 'DEBUG_ONLY' 模式，此模式下只校验数据但不导入数据，数据出错时会以 Toast 的方式提示开发者，请上线前一定使用 DEBUG_OFF 模式。";
                    } else if (mDebugMode == SendataAPI.DebugMode.DEBUG_AND_TRACK) {
                        info = "现在您打开了Smarthub Sendata SDK 的 'DEBUG_AND_TRACK' 模式，此模式下校验数据并且导入数据，数据出错时会以 Toast 的方式提示开发者，请上线前一定使用 DEBUG_OFF 模式。";
                    }
    ...
    private void trackEventInternal(final EventType eventType, final String eventName, final JSONObject properties, final JSONObject sendProperties,
        ...
        if (null != properties) {
            ...
            SendataUtils.mergeJSONObject(properties, sendProperties);
            if (eventType.isTrack()) {
        ...
        if (mAutoTrack && properties != null) {
            if (SendataAPI.AutoTrackEventType.isAutoTrackType(eventName)) {
                SendataAPI.AutoTrackEventType trackEventType = SendataAPI.AutoTrackEventType.autoTrackEventTypeFromEventName(eventName);
    ...
    private JSONArray getPluginVersion() {
        try {
            if (!TextUtils.isEmpty(SendataAPI.ANDROID_PLUGIN_VERSION)) {
                SALog.i(TAG, "android plugin version: " + SendataAPI.ANDROID_PLUGIN_VERSION);
                JSONArray libPluginVersion = new JSONArray();
                libPluginVersion.put("android:" + SendataAPI.ANDROID_PLUGIN_VERSION);
    ...
    private void registerObserver() {
        ...
        SendataContentObserver contentObserver = new SendataContentObserver();
    ...
    private void getCarrier(JSONObject property) {
        try {
            if (TextUtils.isEmpty(property.optString("$carrier")) && mSAConfigOptions.isDataCollectEnable) {
                String carrier = SendataUtils.getCarrier(mContext);
