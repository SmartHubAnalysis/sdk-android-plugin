...
import com.sendata.analytics.android.sdk.data.persistent.PersistentRemoteSDKConfig;
 ...
 */
public class SendataRemoteManager extends BaseSendataSDKRemoteManager {

    private static final String SHARED_PREF_REQUEST_TIME = "sendata.request.time";
    private static final String SHARED_PREF_REQUEST_TIME_RANDOM = "sendata.request.time.random";
    private static final String TAG = "SA.SendataRemoteManager";
    ...
    public SendataRemoteManager(
            SendataAPI sendataAPI) {
        super(sendataAPI);
        ...
        SALog.i(TAG, "Construct a SendataRemoteManager");
    }
    ...
    public void pullSDKConfigFromServer() {
        ...
        if (mSendataEncrypt != null && mSendataEncrypt.isPublicSecretKeyNull()) {
    ...
    public void requestRemoteConfig(RandomTimeType randomTimeType, final boolean enableConfigV) {
        if (mSendataAPI != null && !mSendataAPI.isNetworkRequestEnable()) {
                ...
                requestRemoteConfig(enableConfigV, new HttpCallback.StringCallback() {
                    ...
                    public void onResponse(String response) {
                        ...
                        if (!TextUtils.isEmpty(response)) {
                            SendataSDKRemoteConfig sdkRemoteConfig = toSDKRemoteConfig(response);
                            try {
                                if (mSendataEncrypt != null) {
                                    mSendataEncrypt.saveSecretKey(sdkRemoteConfig.getSecretKey());
                                }
                            } catch (Exception e) {
    ...
    /**
     * 更新 SendataSDKRemoteConfig
     *
     * @param sdkRemoteConfig SendataSDKRemoteConfig 在线控制 SDK 的配置
     */
    @Override
    protected void setSDKRemoteConfig(SendataSDKRemoteConfig sdkRemoteConfig) {
        try {
            ...
            SendataAPI.sharedInstance().trackInternal("$AppRemoteConfigChanged", eventProperties);
    ...
    public void applySDKConfigFromCache() {
        try {
            SendataSDKRemoteConfig sdkRemoteConfig = toSDKRemoteConfig(mPersistentRemoteSDKConfig.get());
            ...
            if (mSendataAPI != null) {
                ...
                if (sdkRemoteConfig.isDisableDebugMode()) {
                    mSendataAPI.setDebugMode(SendataAPI.DebugMode.DEBUG_OFF);
    ...
    private SharedPreferences getSharedPreferences() {
        if (this.mSharedPreferences == null) {
            this.mSharedPreferences = SendataUtils.getSharedPreferences(mContext);
        }
