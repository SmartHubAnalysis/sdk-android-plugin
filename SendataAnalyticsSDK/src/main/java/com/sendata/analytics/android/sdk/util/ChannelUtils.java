...
import static com.sendata.analytics.android.sdk.util.SendataUtils.getSharedPreferences;

public class ChannelUtils {
    private static final String UTM_SOURCE_KEY = "SENDATA_ANALYTICS_UTM_SOURCE";
    private static final String UTM_MEDIUM_KEY = "SENDATA_ANALYTICS_UTM_MEDIUM";
    private static final String UTM_TERM_KEY = "SENDATA_ANALYTICS_UTM_TERM";
    private static final String UTM_CONTENT_KEY = "SENDATA_ANALYTICS_UTM_CONTENT";
    private static final String UTM_CAMPAIGN_KEY = "SENDATA_ANALYTICS_UTM_CAMPAIGN";
    private static final String SHARED_PREF_UTM_FILE = "sendata.utm";
    private static final String SHARED_PREF_CORRECT_TRACK_INSTALLATION = "sendata.correct.track.installation";
    ...
    public static String getDeviceInfo(Context mContext, String androidId, String oaid) {
        return String.format("android_id=%s##imei=%s##imei_old=%s##imei_slot1=%s##imei_slot2=%s##imei_meid=%s##mac=%s##oaid=%s",
                androidId,
                SendataUtils.getIMEI(mContext),
                SendataUtils.getIMEIOld(mContext),
                SendataUtils.getSlot(mContext, 0),
                SendataUtils.getSlot(mContext, 1),
                SendataUtils.getMEID(mContext),
                SendataUtils.getMacAddress(mContext),
                oaid);
    }
    ...
    public static boolean isGetDeviceInfo(Context context, String androidId, String oaid) {
        try {
            return !TextUtils.isEmpty(androidId) ||
                    ...
                    !TextUtils.isEmpty(SendataUtils.getIMEI(context)) ||
                    !TextUtils.isEmpty(SendataUtils.getIMEIOld(context)) ||
                    !TextUtils.isEmpty(SendataUtils.getSlot(context, 0)) ||
                    !TextUtils.isEmpty(SendataUtils.getSlot(context, 1)) ||
                    !TextUtils.isEmpty(SendataUtils.getMEID(context));
        } catch (Exception e) {
            SALog.printStackTrace(e);
        }
        return false;
    ...
    public static boolean checkDeviceInfo(Context context, String deviceInfo) {
        ...
        return (deviceMaps.containsKey("oaid")//防止都为 null 返回 true
                       ...
                       (deviceMaps.containsKey("imei") &&
                                TextUtils.equals(deviceMaps.get("imei"), SendataUtils.getIMEI(context))) ||
                       (deviceMaps.containsKey("imei_old") &&
                                TextUtils.equals(deviceMaps.get("imei_old"), SendataUtils.getIMEIOld(context))) ||
                       (deviceMaps.containsKey("imei_slot1") &&
                                TextUtils.equals(deviceMaps.get("imei_slot1"), SendataUtils.getSlot(context, 0))) ||
                       (deviceMaps.containsKey("imei_slot2") &&
                                TextUtils.equals(deviceMaps.get("imei_slot2"), SendataUtils.getSlot(context, 1))) ||
                       (deviceMaps.containsKey("imei_meid") &&
                                TextUtils.equals(deviceMaps.get("imei_meid"), SendataUtils.getMEID(context))) ||
                       (deviceMaps.containsKey("android_id") &&
                                TextUtils.equals(deviceMaps.get("android_id"), SendataUtils.getAndroidID(context))) ||
                       (deviceMaps.containsKey("mac") &&
                                TextUtils.equals(deviceMaps.get("mac"), SendataUtils.getMacAddress(context)));
    }
}
