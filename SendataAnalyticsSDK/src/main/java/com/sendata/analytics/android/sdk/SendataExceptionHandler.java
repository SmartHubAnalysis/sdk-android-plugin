...
class SendataExceptionHandler implements Thread.UncaughtExceptionHandler {
    private static final int SLEEP_TIMEOUT_MS = 500;

    private static SendataExceptionHandler sInstance;
    private Thread.UncaughtExceptionHandler mDefaultExceptionHandler;
    private static boolean isTrackCrash = false;

    private SendataExceptionHandler() {
    ...
    synchronized static void init() {
        if (sInstance == null) {
            sInstance = new SendataExceptionHandler();
        }
    }
        ...
        try {
            if (isTrackCrash) {
                try {
                    ...
                    SendataAPI.sharedInstance().trackEvent(EventType.TRACK, "AppCrashed", messageProp, null);
                } catch (Exception ex) {
                    SALog.printStackTrace(ex);
                }
            }
            ...
            if (SendataAPI.sharedInstance().isMultiProcessFlushData()) {
