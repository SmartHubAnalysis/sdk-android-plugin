...
public class DbAdapter {
    ...
    private DbAdapter(Context context, String packageName, SendataEncrypt sendataEncrypt) {
        mDbParams = DbParams.getInstance(packageName);
        if (sendataEncrypt != null) {
            mTrackEventOperation = new EncryptDataOperation(context.getApplicationContext(), sendataEncrypt);
        } else {
    ...
    public static DbAdapter getInstance(Context context, String packageName,
                                        SendataEncrypt sendataEncrypt) {
        if (instance == null) {
            instance = new DbAdapter(context, packageName, sendataEncrypt);
        }
        return instance;
    }
