...
public class DbParams {
    ...
    /* 数据库名称 */
    public static final String DATABASE_NAME = "sendata";
    /* 数据库版本号 */
    ...
    private DbParams(String packageName) {
        mUri = Uri.parse("content://" + packageName + ".SendataContentProvider/" + TABLE_EVENTS);
        mActivityStartCountUri = Uri.parse("content://" + packageName + ".SendataContentProvider/" + TABLE_ACTIVITY_START_COUNT);
        mAppStartTimeUri = Uri.parse("content://" + packageName + ".SendataContentProvider/" + TABLE_APP_START_TIME);
        mAppEndDataUri = Uri.parse("content://" + packageName + ".SendataContentProvider/" + TABLE_APP_END_DATA);
        mAppEndUri = Uri.parse("content://" + packageName + ".SendataContentProvider/" + TABLE_APP_END_TIME);
        mSessionTimeUri = Uri.parse("content://" + packageName + ".SendataContentProvider/" + TABLE_SESSION_INTERVAL_TIME);
        mLoginIdUri = Uri.parse("content://" + packageName + ".SendataContentProvider/" + TABLE_LOGIN_ID);
        mChannelPersistentUri = Uri.parse("content://" + packageName + ".SendataContentProvider/" + TABLE_CHANNEL_PERSISTENT);
        mSubProcessUri = Uri.parse("content://" + packageName + ".SendataContentProvider/" + TABLE_SUB_PROCESS_FLUSH_DATA);
        mFirstProcessUri = Uri.parse("content://" + packageName + ".SendataContentProvider/" + TABLE_FIRST_PROCESS_START);
        mDataCollectUri = Uri.parse("content://" + packageName + ".SendataContentProvider/" + TABLE_DATA_COLLECT);
    }
