...
import com.sendata.analytics.android.sdk.exceptions.ConnectErrorException;
import com.sendata.analytics.android.sdk.exceptions.DebugModeException;
import com.sendata.analytics.android.sdk.exceptions.InvalidDataException;
import com.sendata.analytics.android.sdk.exceptions.ResponseErrorException;
import com.sendata.analytics.android.sdk.util.Base64Coder;
...
/**
 * Manage communication of events with the internal database and the Sendata servers.
 * This class straddles the thread boundary between user threads and
 * a logical Sendata thread.
 */
class AnalyticsMessages {
    ...
    private SendataAPI mSendataAPI;

    /**
     * 不要直接调用，通过 getInstance 方法获取实例
     */
    private AnalyticsMessages(final Context context, SendataAPI sendataAPI) {
        mContext = context;
        mDbAdapter = DbAdapter.getInstance();
        mWorker = new Worker();
        mSendataAPI = sendataAPI;
    }
     ...
     */
    public static AnalyticsMessages getInstance(final Context messageContext, final SendataAPI sendataAPI) {
        synchronized (S_INSTANCES) {
            final Context appContext = messageContext.getApplicationContext();
            final AnalyticsMessages ret;
            if (!S_INSTANCES.containsKey(appContext)) {
                ret = new AnalyticsMessages(appContext, sendataAPI);
    ...
    void enqueueEventMessage(final String type, final JSONObject eventJson) {
        try {
            synchronized (mDbAdapter) {
                ...
                if (mSendataAPI.isDebugMode() || ret ==
                        DbParams.DB_OUT_OF_MEMORY_ERROR) {
                    mWorker.runMessage(m);
                } else {
                    // track_signup 立即发送
                    if (type.equals("track_signup") || ret > mSendataAPI
                            .getFlushBulkSize()) {
                        mWorker.runMessage(m);
                    } else {
                        final int interval = mSendataAPI.getFlushInterval();
    ...
    void flushScheduled() {
        try {
            ...
            mWorker.runMessageOnce(m, mSendataAPI.getFlushInterval());
        } catch (Exception e) {
            SALog.printStackTrace(e);
        }
    }
    ...
    private void sendData() {
        try {
            if (!mSendataAPI.isNetworkRequestEnable()) {
                SALog.i(TAG, "NetworkRequest 已关闭，不发送数据！");
                return;
            }

            if (TextUtils.isEmpty(mSendataAPI.getServerUrl())) {
        ...
        while (count > 0) {
            ...
            try {
                ...
                if (!TextUtils.isEmpty(data)) {
                    sendHttpRequest(mSendataAPI.getServerUrl(), data, gzip, rawMessage, false);
                }
                ...
                boolean isDebugMode = mSendataAPI.isDebugMode();
                if (!TextUtils.isEmpty(errorMessage)) {
                    if (isDebugMode || SALog.isLogEnabled()) {
                        SALog.i(TAG, errorMessage);
                        if (isDebugMode && SendataAPI.SHOW_DEBUG_INFO_VIEW) {
                            try {
                                /*
    ...
    private void sendHttpRequest(String path, String data, String gzip, String rawMessage, boolean isRedirects) throws ConnectErrorException, ResponseErrorException {
        ...
        try {
            ...
            if (mSendataAPI.getDebugMode() == SendataAPI.DebugMode.DEBUG_ONLY) {
                connection.addRequestProperty("Dry-Run", "true");
            }

            String cookie = mSendataAPI.getCookie(false);
    ...
    private class Worker {
        ...
        Worker() {
            final HandlerThread thread =
                    new HandlerThread("com.sendata.analytics.android.sdk.AnalyticsMessages.Worker",
                            Thread.MIN_PRIORITY);
            thread.start();
            ...
            public void handleMessage(Message msg) {
                try {
                    if (msg.what == FLUSH_QUEUE) {
                        ...
                        SALog.i(TAG, "Unexpected message received by Sendata worker: " + msg);
                    }
