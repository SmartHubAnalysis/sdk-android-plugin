...
public class WebViewVisualInterface {
    ...
    /**
     * JS 给 App 提供 H5 页面数据（只有当 sendata_visualized_mode = true 时 JS 才返回数据）
     *
     * @param msg H5 页面数据
     */
    @JavascriptInterface
    public void sendata_hover_web_nodes(final String msg) {
        try {
            SALog.i(TAG, "sendata_hover_web_nodes msg: " + msg);
     ...
     */
    @JavascriptInterface
    public boolean sendata_visualized_mode() {
        return VisualizedAutoTrackService.getInstance().isServiceRunning() || HeatMapService.getInstance().isServiceRunning();
    }


    @JavascriptInterface
    public void sendata_visualized_alert_info(final String msg) {
        try {
            SALog.i(TAG, "sendata_visualized_alert_info msg: " + msg);
                ...
                mWebView.get().post(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (!TextUtils.isEmpty(url)) {
                            SALog.i(TAG, "sendata_visualized_alert_info url: " + url);
