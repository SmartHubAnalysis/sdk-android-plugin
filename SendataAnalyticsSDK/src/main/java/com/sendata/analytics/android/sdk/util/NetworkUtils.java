...
public class NetworkUtils {
    ...
    public static String networkType(Context context) {
        try {
            ...
            // 检测权限
            if (!SendataUtils.checkHasPermission(context, Manifest.permission.ACCESS_NETWORK_STATE)) {
                networkType = "NULL";
                return networkType;
            }
    ...
    private static int toNetworkType(String networkType) {
        if ("NULL".equals(networkType)) {
            return SendataAPI.NetworkType.TYPE_ALL;
        } else if ("WIFI".equals(networkType)) {
            return SendataAPI.NetworkType.TYPE_WIFI;
        } else if ("2G".equals(networkType)) {
            return SendataAPI.NetworkType.TYPE_2G;
        } else if ("3G".equals(networkType)) {
            return SendataAPI.NetworkType.TYPE_3G;
        } else if ("4G".equals(networkType)) {
            return SendataAPI.NetworkType.TYPE_4G;
        } else if ("5G".equals(networkType)) {
            return SendataAPI.NetworkType.TYPE_5G;
        }
        return SendataAPI.NetworkType.TYPE_ALL;
    }
    ...
    public static void registerNetworkListener(Context context) {
        try {
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
                SendataNetworkListener.SABroadcastReceiver mReceiver = new SendataNetworkListener.SABroadcastReceiver();
                IntentFilter intentFilter = new IntentFilter();
                intentFilter.addAction(ConnectivityManager.CONNECTIVITY_ACTION);
                context.registerReceiver(mReceiver, intentFilter);
                SALog.i(TAG, "Register BroadcastReceiver");
            } else {
                SendataNetworkListener.SANetworkCallbackImpl networkCallback = new SendataNetworkListener.SANetworkCallbackImpl();
    ...
    private static String mobileNetworkType(Context context, TelephonyManager telephonyManager, ConnectivityManager connectivityManager) {
        ...
        if (telephonyManager != null) {
            if (android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.R
                    && (SendataUtils.checkHasPermission(context, Manifest.permission.READ_PHONE_STATE) || telephonyManager.hasCarrierPrivileges())) {
