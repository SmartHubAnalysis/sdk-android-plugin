...
package com.sendata.analytics.android.sdk.remote;
...
import com.sendata.analytics.android.sdk.encrypt.SecreteKey;
...
public abstract class BaseSendataSDKRemoteManager {

    protected static final String TAG = "SA.SendataSDKRemoteConfigBase";
    protected Context mContext;
    protected SAConfigOptions mSAConfigOptions;
    protected SendataEncrypt mSendataEncrypt;
    protected boolean mDisableDefaultRemoteConfig;

    protected static SendataSDKRemoteConfig mSDKRemoteConfig;
    protected SendataAPI mSendataAPI;

    protected BaseSendataSDKRemoteManager(SendataAPI sendataAPI) {
        this.mSendataAPI = sendataAPI;
        this.mContext = sendataAPI.getContext();
        this.mSAConfigOptions = sendataAPI.getConfigOptions();
        this.mSendataEncrypt = sendataAPI.getSendataEncrypt();
        this.mDisableDefaultRemoteConfig = sendataAPI.isDisableDefaultRemoteConfig();
    }
    ...
    protected abstract void setSDKRemoteConfig(SendataSDKRemoteConfig sdkRemoteConfig);
    ...
    /**
     * 将 json 格式的字符串转成 SendataSDKRemoteConfig 对象，并处理默认值
     *
     * @param config String
     * @return SendataSDKRemoteConfig
     */
    protected SendataSDKRemoteConfig toSDKRemoteConfig(String config) {
        SendataSDKRemoteConfig sdkRemoteConfig = new SendataSDKRemoteConfig();
        try {
            if (!TextUtils.isEmpty(config)) {
                ...
                if (!TextUtils.isEmpty(configs)) {
                    ...
                    if (keyObject != null) {
                        if (keyObject.has("key_ec") && SendataEncrypt.isECEncrypt()) {
    ...
    public Boolean isAutoTrackEventTypeIgnored(int autoTrackEventType) {
        if (mSDKRemoteConfig != null) {
            if (mSDKRemoteConfig.getAutoTrackMode() != SendataSDKRemoteConfig.REMOTE_EVENT_TYPE_NO_USE) {
    ...
    protected String buildRemoteUrl(boolean enableConfigV) {
        ...
        String serverUlr = mSendataAPI.getServerUrl();
        ...
        if (!TextUtils.isEmpty(configOptionsRemoteUrl)
            ...
            SALog.i(TAG, "SendataAPI remote url is " + remoteUrl);
        } else {
        ...
        if (configV && (SendataUtils.checkVersionIsNew(mContext, mSendataAPI.getSDKVersion()) ||
                (mSendataEncrypt != null && mSendataEncrypt.isPublicSecretKeyNull()))) {
            configV = false;
        }
        ...
        if (!TextUtils.isEmpty(remoteUrl) && configV) {
            ...
            SendataSDKRemoteConfig SDKRemoteConfig = mSDKRemoteConfig;
