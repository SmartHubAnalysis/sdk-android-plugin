...
import com.sendata.analytics.android.sdk.ThreadNameConstants;
import com.sendata.analytics.android.sdk.network.HttpCallback;
import com.sendata.analytics.android.sdk.network.HttpMethod;
import com.sendata.analytics.android.sdk.network.RequestHelper;
...
import com.sendata.analytics.android.sdk.visual.view.PairingCodeEditDialog;
...
public class SendataDialogUtils {
    private static final String TAG = "SA.SendataDialogUtils";
    ...
    public static void showChannelDebugDialog(final Activity activity,
        ...
        SendataDialogUtils.showDialog(activity, "即将开启联调模式", "", "确定", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                ...
                if (!isTrackInstallation || ChannelUtils.isCorrectTrackInstallation(context)) {
                    String androidId = SendataUtils.getAndroidID(context);
                    ...
                    final SendataLoadingDialog loadingDialog = new SendataLoadingDialog(activity);
                            ...
                            new HttpCallback.JsonCallback() {
                                ...
                                public void onResponse(JSONObject response) {
                                    ...
                                    if (response == null) {
                                        ...
                                        showDialog(activity, "添加白名单请求失败，请联系Smarthub技术支持人员排查问题!");
                                        return;
                                    }
    ...
    private static void showChannelDebugErrorDialog(final Activity activity) {
        SendataDialogUtils.showDialog(activity, "检测到 “设备码为空”，可能原因如下，请排查：",
                "1. 开启 App 时拒绝“电话”授权；\n" +
    ...
    public static void showPopupWindowDialog(Activity activity, Uri uri) {
        try {
            Class<?> clazz = Class.forName("com.sendata.sf.ui.utils.PreviewUtil");
    ...
    public static void showDebugModeSelectDialog(final Activity activity, final String infoId, final String locationHref, final String project) {
        try {
            DebugModeSelectDialog dialog = new DebugModeSelectDialog(activity, SendataAPI.sharedInstance().getDebugMode());
            ...
            dialog.setOnDebugModeDialogClickListener(new DebugModeSelectDialog.OnDebugModeViewClickListener() {
                ...
                @Override
                public void setDebugMode(Dialog dialog, SendataAPI.DebugMode debugMode) {
                    SendataAPI.sharedInstance().setDebugMode(debugMode);
                    dialog.cancel();
                }
            });
            dialog.setOnCancelListener(new DialogInterface.OnCancelListener() {
                ...
                public void onCancel(DialogInterface dialog) {
                    ...
                    SendataAPI.DebugMode mCurrentDebugMode = SendataAPI.sharedInstance().getDebugMode();
                    if (SendataAPI.sharedInstance().isNetworkRequestEnable() && !TextUtils.isEmpty(serverUrl) && !TextUtils.isEmpty(infoId) && mCurrentDebugMode != SendataAPI.DebugMode.DEBUG_OFF) {
                        if (TextUtils.isEmpty(locationHref)) {
                            new SendDebugIdThread(serverUrl, SendataAPI.sharedInstance().getDistinctId(), infoId, ThreadNameConstants.THREAD_SEND_DISTINCT_ID).start();
                        } else {
                            try {
                                if (!TextUtils.isEmpty(project)) {
                                    String url = locationHref + "?project=" + project;
                                    SALog.i(TAG, "sf url:" + url);
                                    new SendDebugIdThread(url, SendataAPI.sharedInstance().getDistinctId(), infoId, ThreadNameConstants.THREAD_SEND_DISTINCT_ID).start();
                                }
                            } catch (Exception e) {
                                SALog.printStackTrace(e);
                            }
                    ...
                    if (mCurrentDebugMode == SendataAPI.DebugMode.DEBUG_OFF) {
                        currentDebugToastMsg = "已关闭调试模式，请重新扫描二维码进行开启";
                    } else if (mCurrentDebugMode == SendataAPI.DebugMode.DEBUG_ONLY) {
                        currentDebugToastMsg = "开启调试模式，校验数据，但不进行数据导入；关闭 App 进程后，将自动关闭调试模式";
                    } else if (mCurrentDebugMode == SendataAPI.DebugMode.DEBUG_AND_TRACK) {
                        currentDebugToastMsg = "开启调试模式，校验数据，并将数据导入到Smarthub分析中；关闭 App 进程后，将自动关闭调试模式";
                    }
    ...
    public static void showOpenHeatMapDialog(final Activity context, final String featureCode,
        ...
        try {
            if (!SendataAPI.sharedInstance().isNetworkRequestEnable()) {
                showDialog(context, "已关闭网络请求（NetworkRequest），无法使用 App 点击分析，请开启后再试！");
                return;
            }

            if (!SendataAPI.sharedInstance().isHeatMapEnabled()) {
                showDialog(context, "SDK 没有被正确集成，请联系贵方技术人员开启点击分析。");
                return;
            }

            if (!SendataAPI.sharedInstance().isAppHeatMapConfirmDialogEnabled()) {
    ...
    public static void showOpenVisualizedAutoTrackDialog(final Activity context, final String featureCode, final String postUrl) {
        try {
            ...
            if (!SendataAPI.sharedInstance().isVisualizedAutoTrackEnabled()) {
                showDialog(context, "SDK 没有被正确集成，请联系贵方技术人员开启可视化全埋点。");
                return;
            }
            if (!SendataAPI.sharedInstance().isVisualizedAutoTrackConfirmDialogEnabled()) {
    ...
    private static void requestActiveChannel(String baseUrl, String monitorId,
        ...
        try {
            ...
            json.put("distinct_id", SendataAPI.sharedInstance().getDistinctId());
