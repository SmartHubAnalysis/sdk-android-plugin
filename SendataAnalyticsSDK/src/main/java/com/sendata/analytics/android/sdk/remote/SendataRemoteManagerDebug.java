...
import com.sendata.analytics.android.sdk.dialog.SendataLoadingDialog;
 ...
 * SDK 调试采集控制时，调试管理类
 */
public class SendataRemoteManagerDebug extends BaseSendataSDKRemoteManager {

    private static final String TAG = "SA.SendataRemoteManagerDebug";
    private String errorMsg = "";

    public SendataRemoteManagerDebug(SendataAPI sendataAPI) {
        super(sendataAPI);
        SALog.i(TAG, "remote config: Construct a SendataRemoteManagerDebug");
    }
    ...
    @Override
    public void setSDKRemoteConfig(SendataSDKRemoteConfig sdkRemoteConfig) {
        try {
    ...
    public void checkRemoteConfig(final Uri uri, final Activity activity) {
        if (verifyRemoteRequestParameter(uri, activity)) {
            SendataDialogUtils.showDialog(activity, "提示",
                    "开始获取采集控制信息", "继续", new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            SendataDialogUtils.dialogShowDismissOld(loadingDialog);
                            // 发起请求
                            requestRemoteConfig(false, new HttpCallback.StringCallback() {
                                @Override
                                public void onFailure(int code, String errorMessage) {
                                    loadingDialog.dismiss();
                                    SendataDialogUtils.showDialog(activity, "远程配置获取失败，请稍后重新扫描二维码");
                                ...
                                public void onResponse(String response) {
                                    ...
                                    if (!TextUtils.isEmpty(response)) {
                                        ...
                                        if (!sdkRemoteConfig.getNewVersion().equals(nv)) {
                                            SendataDialogUtils.showDialog(activity, "信息版本不一致", "获取到采集控制信息的版本：" +
                                            ...
                                            SendataDialogUtils.showDialog(activity, "采集控制加载完成，可以通过 Android Studio 控制台日志来调试");
                                            setSDKRemoteConfig(sdkRemoteConfig);
                                        }
                                    } else {
                                        SendataDialogUtils.showDialog(activity, "远程配置获取失败，请稍后再试");
                                    }
            ...
            // 没有校验通过
            SendataDialogUtils.showDialog(activity, errorMsg);
        }
    }
    ...
    private boolean verifyRemoteRequestParameter(Uri uri, Activity activity) {
        ...
        String nv = uri.getQueryParameter("nv");
        String localProject = "";
        String serverUrl = mSendataAPI.getServerUrl();
        ...
        } else if (mSendataAPI != null && !mSendataAPI.isNetworkRequestEnable()) {
